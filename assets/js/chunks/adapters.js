...
import { n as qselAll, f as escapeHtml, k as icon, j as hasFeature, r as qselFrom, c as assetPlan } from "./shared.js";
...
function renderCode() {
  var _a;
  if (typeof ((_a = window.hljs) == null ? void 0 : _a.highlightElement) !== "function") {
    return;
  }
...
function highlightBlobCode(code) {
  ...
  if (!hasLanguage || typeof ((_a = window.hljs) == null ? void 0 : _a.highlightElement) !== "function") {
    return;
  }
  window.hljs.highlightElement(code);
  normalizeShellHighlight(code);
...
function openTag(node) {
  const attrs = [...node.attributes].map((attr) => `${attr.name}="${escapeHtml(attr.value)}"`).join(" ");
  return attrs ? `<${node.tagName.toLowerCase()} ${attrs}>` : `<${node.tagName.toLowerCase()}>`;
}
...
function isShellCode(code) {
  return [...code.classList].some(
    (name) => ["language-bash", "language-sh", "language-shell"].includes(name)
  );
}
...
function getCopyHost(pre) {
  ...
  if (wrapper == null ? void 0 : wrapper.classList.contains("highlight")) {
    return wrapper;
  }
  return pre;
}
function getCopyText(pre) {
  var _a;
  return ((_a = pre.querySelector("code")) == null ? void 0 : _a.textContent) || pre.textContent || "";
}
...
function renderMapBlock(block, kind) {
  ...
  const layer = window.L.geoJSON(geojson, {
    style(feature) {
      var _a;
      const type = ((_a = feature == null ? void 0 : feature.geometry) == null ? void 0 : _a.type) || "";
...
function restoreGitHubInlineMath(container) {
  for (const code of container.querySelectorAll("code")) {
    ...
    if ((previous == null ? void 0 : previous.nodeType) !== Node.TEXT_NODE || (next == null ? void 0 : next.nodeType) !== Node.TEXT_NODE) {
      continue;
    }
...
async function getMermaidVersion(api) {
  ...
  if (!mermaidVersionPromise) {
    ...
    mermaidVersionPromise = fetch(versionPath).then((r) => r.text()).then((t) => t.trim() || "unknown").catch(() => "unknown");
  }
